...
from hypothesisEngine.utilities.algorithm.NSGA2 import compute_pareto_metrics
from hypothesisEngine.utilities.algorithm.state import create_state
from hypothesisEngine.utilities.stats import trackers
from hypothesisEngine.utilities.stats.save_plots import save_plot_from_data, \
    save_pareto_fitness_plot
from hypothesisEngine.utilities.stats.file_io import save_stats_to_file, save_stats_headers, \
...
def print_generation_stats():
    ...
    for stat in sorted(stats.keys()):
        ...
        ps.print_list.append(" "+stat + ": \t" +str(stats[stat]))
    print("\n")
...
def print_final_stats():
    ...
    if hasattr(params['FITNESS_FUNCTION'], "training_test"):
        ...
        ps.print_list.append("\n\nBest:\n  Training fitness:\t"+str(trackers.best_ever.training_fitness))
        print("  Test fitness:\t\t", trackers.best_ever.test_fitness)
        ps.print_list.append("  Test fitness:\t\t"+str(trackers.best_ever.test_fitness))
    else:
        print("\n\nBest:\n  Fitness:\t", trackers.best_ever.fitness)
        ps.print_list.append("\n\nBest:\n  Fitness:\t"+str(trackers.best_ever.fitness))

    print("  Phenotype:", trackers.best_ever.phenotype)
    ps.print_list.append("  Phenotype:"+str(trackers.best_ever.phenotype))
    print("  Genome:", trackers.best_ever.genome)
    ps.print_list.append("  Genome:"+str(trackers.best_ever.genome))
